...
Transnap 是一个 macOS 菜单栏翻译工具，主打“复制即翻”。它基于系统 `Translation` 能力做本地翻译，通过菜单栏面板和全局快捷键快速唤起，适合处理阅读、聊天、文档里的碎片文本。
...
```text
Transnap/
├── TransnapApp.swift                # 应用入口，初始化模型、菜单栏与快捷键
├── ViewModels/
...
1. 应用启动后初始化 SwiftData、设置存储、菜单栏入口和全局快捷键
2. 用户通过菜单栏或快捷键触发翻译
